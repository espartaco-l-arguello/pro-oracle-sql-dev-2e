create or replace package space_exporter authid current_user is
...
procedure generate_oracle_file is
	...
	begin
		v_handle := utl_file.fopen('SPACE_OUTPUT_DIR', 'oracle_create_space.sql', open_mode => p_open_mode);
		return v_handle;
	end get_handle;
...
procedure generate_postgres_file is
	...
	function get_handle(p_open_mode in varchar2) return utl_file.file_type is
		v_handle utl_file.file_type;
	begin
		v_handle := utl_file.fopen('SPACE_OUTPUT_DIR', 'postgres_create_space.sql', open_mode => p_open_mode);
		return v_handle;
	end get_handle;
	...
	begin
		...
		q'[
				...
				-- Print intro message.
				\echo ------------------------------------------------------------------------
				\echo -- Installing space database version #VERSION#, generated on #DATE#.
				\echo --
	...
	begin
		...
		for i in 1 .. g_ordered_objects.count loop
			...
			--Metadata for tables:
			v_metadata := get_metadata('TABLE', 'SPACE'/*temp*/, g_ordered_objects(i)) || ';';
